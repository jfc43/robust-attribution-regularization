...
class IntegratedGradientsAttack(object):
    ...
    def __init__(self, sess, test_image, original_label, NET, attack_method, epsilon,
                 k_top=100, eval_k_top=100, num_steps=100, attack_iters=100, attack_times=3,
                 alpha=1.,  attack_measure="intersection",
    ...
    def create_attack_ops(self):
        ...
        self.saliency1, self.topK = self.run_model(self.sess,[self.NET.saliency, self.NET.top_idx], counterfactuals, self.NET)
        ...
        if self.attack_method == 'mass_center' or self.attack_measure == 'mass_center':
            ...
            counterfactuals = self.create_counterfactuals(self.test_image)
            self.mass_center1 = self.run_model(self.sess, self.NET.mass_center2, counterfactuals, self.NET).astype(int)
        ...
        if self.target_map is not None:
            ...
            NET.target_direction = -tf.gradients(target_loss, self.NET.input)[0]
    ...
    def give_simple_perturbation(self, attack_method, in_image):
        ...
        if attack_method == "random":
            perturbation = np.random.normal(size=(self.num_steps, self.w, self.h, self.c))
        elif attack_method == "topK":
            perturbation = self.run_model(self.sess, self.NET.topK_direction, counterfactuals, self.NET)
            perturbation = np.reshape(perturbation, [self.num_steps, self.w, self.h, self.c])
        elif attack_method == "mass_center":
            perturbation = self.run_model(self.sess, self.NET.mass_center_direction, counterfactuals, self.NET)
            perturbation = np.reshape(perturbation, [self.num_steps, self.w, self.h, self.c])
        elif attack_method == "target":
            if self.target_map is None:
                raise ValueError("No target region determined!")
            else:
                perturbation = self.run_model(self.sess, self.NET.target_direction, counterfactuals, self.NET)
                perturbation = np.reshape(perturbation, [self.num_steps, self.w, self.h, self.c])
    ...
    def apply_perturb(self, in_image, pert, alpha, bound=8.0):

        out_image = self.test_image + np.clip(in_image + alpha * pert - self.test_image, -bound, bound)
        out_image = np.clip(out_image, 0.0, 255.0)
        out_image = out_image.astype(np.int32)
        return out_image
    ...
    def iterative_attack_once(self):
        ...
        self.saliency2 = self.run_model(self.sess, [self.NET.saliency], counterfactuals, self.NET)
