from flower_input import load_data
import numpy as np
...
def augment_and_balance_data(X_train, y_train, no_examples_per_class =1000):
    ...
    for c, count in zip(range(17), class_counts):
...
X, Y = load_data(dirname="data/17flowers", resize_pics=(128, 128), shuffle=True, one_hot=False)
...
X_out, Y_out = augment_and_balance_data(train_X, train_Y, 1000)

np.savez_compressed('data/train', images = X_out, labels = Y_out)
np.savez_compressed('data/test', images = test_X, labels = test_Y)
